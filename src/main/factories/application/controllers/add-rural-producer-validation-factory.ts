...
export const makeAddRuralProducerValidation = () => {
  const schema = yup.object().shape({
    ...
    planted_crops: yup
      ...
      .required()
  }).test('cpf-or-cnpj', 'At least one of CPF or CNPJ is required', function (value) {
    const { cpf, cnpj } = value;
    if (!cpf && !cnpj) {
      return this.createError({ message: 'At least one of CPF or CNPJ is required' });
    }
    return true;
  }).test('area-sum', 'The sum of arable area and vegetation area must not exceed the total area', function (value) {
    const { total_area_hectares, arable_area_hectares, vegetation_area_hectares } = value;
    if (total_area_hectares < arable_area_hectares + vegetation_area_hectares) {
      return this.createError({
        message: 'The sum of arable area and vegetation area must not exceed the total area',
      });
    }
    return true;
  });
